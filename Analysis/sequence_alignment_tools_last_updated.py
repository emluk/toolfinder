...
tools_sequence_analysis = db.execute("""SELECT Biotools_id FROM biotools_tools_operations WHERE EDAM_id = 'operation_0292'""").fetchall()
...
plt.title("Last Update on Tools in Sequence Alignment")
plt.bar(sorted_years, sorted_counts)
plt.tight_layout()
plt.savefig('D:\\Priv\\repository\\toolfinder\\Data\\images\\sequence_alignment_last_updated.svg')
plt.show()